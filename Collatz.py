...
def collatz_cache (i, j) :
    ...
    if (j <= 100) :
        ...
        max_cycle_len = meta_cache_helper (i, j, list_less_1000, 100)

    else :
        max_cycle_len = meta_cache_helper (i, j, list_more_1000, 1000)
...
def meta_cache_helper(i, j, cache_list, const_val):
    """
    i the beginning of the range, inclusive
    j the end       of the range, inclusive
    cache_list global cache lists
    ...
    if (start_index == end_index):
        ...
        calc_before = collatz_eval (i, (start_index+1)*const_val)

        for index in range (start_index+1, end_index) :
            if (cache_list[index] > calc_middle):
                calc_middle = cache_list[index]
            has_middle = True

        calc_after = collatz_eval (end_index*const_val, j)

        if (has_middle) :
            max_cycle_len = max(calc_before, calc_middle, calc_after)
        else :
            max_cycle_len = max(calc_before, calc_after)
...
def collatz_solve (r, w) :
    ...
    for s in r :
        i, j = collatz_read(s)
        #v    = collatz_eval(i, j)          #enable only lazy-cache
        v = collatz_cache(i, j)             #enable meta-cache
