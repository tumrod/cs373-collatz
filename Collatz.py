...
def collatz_cache (i, j) :
    ...
    if ((i == j) or (i < 100 or j < 100)) :
        ...
        r = j%100
        r2 = (i-(i%100)) + 100
        # within the same range i.e. 201 - 210
        if (start == end) :
            max_len = collatz_eval(i, j)
        else :
            max_len = max(collatz_eval(j-r, j), collatz_eval(i, r2))

            for val in range(start+1, end) :
                current = list_less_1000[val]
                if (current > max_len) :
                    max_len = current
